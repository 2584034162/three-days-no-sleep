...
本项目仅供学习和个人使用。本项目是一个 AI 对话前端工具，不提供任何 AI 模型或 API 服务。

使用者需自行承担使用本项目产生的一切风险和责任，包括但不限于：

- 使用本项目产生的任何费用（如 API 调用费用）
- 部署和使用过程中的数据安全问题
- 用户自行生成的所有内容

本项目不生成、不存储、不审核任何 AI 对话内容。用户生成的内容由用户自行负责，与本项目作者无关。用户须遵守所在地区的法律法规，不得利用本项目生成违法违规内容。
